...
@php $que = DB::table('fmt_mcq_ques')->where('id', $message)->first(); @endphp
<div class="fixed z-10 inset-0 overflow-y-auto hidden" id="modalMCQ{{$que->id}}">
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full relative -mx-8" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <form action="{{route('fmt.mcq.update', $que->id)}}" method="post">
                    ...
                    @endif
                    @csrf
                    <div class="text-xl">Edit MCQ Question {{$message}}</div>
                    ...
                    @php $answers = DB::table('fmt_mcq_ans')->where('question_id', $que->id)->pluck('id'); @endphp
                    @foreach ($answers as $ans)
                        @php $answer = DB::table('fmt_mcq_ans')->where('id', $ans)->first(); @endphp
                        {{--  --}}
